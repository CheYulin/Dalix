...
double ** distanceMatrix(string filename1, int * nb_row)
{
	...
	while (pdb_file.getline(buffer, 1024))
	{
		...
		strncpy(aa_name, buffer + 17, 3); aa_name[3] = '\0';

		strncpy(r_number, buffer + 22, 4); r_number[4] = '\0';
		...
		if (strncmp(buffer, "ATOM", 4) == 0 || strncmp(buffer, "HETATM", 6) == 0)
		{
			...
			if (strncmp(at_name, " CA ", 4) == 0 && buffer[21] == chain)
			{
				...
				{
					strncpy(at_x, buffer + 30, 8); at_x[8] = '\0';
					strncpy(at_y, buffer + 38, 8); at_y[8] = '\0';
					strncpy(at_z, buffer + 46, 8); at_z[8] = '\0';
					cx_ca = atof(at_x);
					cy_ca = atof(at_y);
					cz_ca = atof(at_z);
					found_ca = true;
...
{
	if (tlc == "ALA" || tlc == "CYS" || tlc == "GLY" || tlc == "HIS" || tlc == "ILE" || tlc == "LEU" || tlc == "MET" || tlc == "PRO" || tlc == "SER" || tlc == "THR" || tlc == "VAL")
	{
