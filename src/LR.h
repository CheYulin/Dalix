...
#pragma omp parallel for schedule(dynamic)
					...
					if (need_to_change[row1][col1])
					{
						...
						for (int row2 = 0; row2 < nb_row - row1; row2++)
						{
							if (row2 != 0)
							{
								for (int col2 = 0; col2 < nb_col - col1; col2++)
								{
									...
									else
									{
										...
										if (row2 + row1 < current_problem->up[col1] && row2 + row1 > current_problem->lo[col1])
											dp_mat_test[0] = 0;
										else
							...
							else
							{
								for (int col2 = 0; col2 < nb_col - col1; col2++)
								{
									if (row1 < current_problem->up[col2 + col1] && row1 > current_problem->lo[col1 + col2])
										dp_mat_test[col2] = 0;
									else
		...
		for (int i = 0; i < nb_row; i++)
		{
			...
			{
				dp_mat[i][0] = align_node[i][0]; path_mat[i][0] = -2;
			}
			else
			{
		...
		for (int i = 0; i < nb_col; i++)
		{
			...
			{
				dp_mat[0][i] = align_node[0][i]; path_mat[0][i] = -1;
			}
			else
			{
		...
		while (x_1 != 1 || y_1 != 1)
		{
			...
			{
				align_node_xvalue[x_1 - 1][y_1 - 1] = true; x_1--; y_1--;
				if (x_1 - 1 == 0 || y_1 - 1 == 0)
